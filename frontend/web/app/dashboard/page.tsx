...
export default function DashBoardPage() {
  ...
  const { data: jobsite } = useSWR(user ? `/api/sites/search?id=${user[0].jobsite_id.Valid ? user[0].jobsite_id.Int64 : undefined}` : null, fetchJobsite,)

  if (!identity) { return (<div className='flex items-center justify-center w-screen h-screen'>Loading <Loading /></div>) }

  return (
    <div className="flex flex-col justify-center items-center w-screen">
      <Dashboard route={user ? `/api/sites/search?id=${user[0].jobsite_id.Valid ? user[0].jobsite_id.Int64 : undefined}` : undefined} header="Jobsite" jobsite={jobsite} />
    </div>
  );
}
