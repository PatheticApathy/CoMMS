...
const Item = ({ coworkers }: { coworkers: Coworker }) => {
  ...
  return (
    <View style={{ ...style.segment, ...color }}>
      ...
      <Text style={{ ...style.text, ...color_text }}>
        {coworkers.firstname.Valid
          ? coworkers.firstname.String + " " + (coworkers.lastname.Valid ? coworkers.lastname.String : "")
          : "No name"}
      </Text>
...
export default function CoworkerList({ coworkers, onRefresh, refreshing }: UserListInput) {
  return (
    <FlatList
      data={coworkers}
